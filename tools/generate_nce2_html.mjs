...
const srcDir = path.join(root, 'NCE2');
...
const endLesson = Number(process.argv[3] ?? 96);
...
function parseLrc(file) {
  ...
  const question = timedLines[0]?.text ?? '';
  const bodyEntries = timedLines.slice(1);
  const fullTextParts = [];
  const segments = [];
  let cursor = 0;
  ...
  return {
    file,
    lessonNo: Number(file.match(/^(\d+)/)?.[1] ?? 0),
    title,
    question,
...
词: ${item.words}`).join('\n\n');
  ...
  return `# NCE2 Lesson ${String(lesson.lessonNo).padStart(2, '0')} - ${lesson.title}
...
function renderHtml(lesson, analyses, previousHref = null, nextHref = null) {
  const title = `NCE2 Lesson ${String(lesson.lessonNo).padStart(2, '0')} - ${lesson.title}`;
...
<head>
  ...
  <style>
    * { box-sizing: border-box; }
    ...
    body { margin: 0; padding-bottom: 156px; font-family: "Microsoft YaHei", "PingFang SC", Arial, sans-serif; color: #1f2937; background: #f3f4f6; line-height: 1.65; }
    ...
    @media (max-width: 760px) { body { padding-bottom: 176px; } .page-header .inner, main { width: min(100% - 20px, 1080px); } .sentence-card, .summary-section { padding: 14px; } .sentence-head { grid-template-columns: 30px 30px 30px 1fr; gap: 8px; } .sentence-play, .sentence-toggle, .sentence-num { width: 30px; height: 30px; } .en-text { font-size: 16px; } .field { grid-template-columns: 1fr; gap: 2px; } .back-top, .back-prev { right: 10px; bottom: 184px; min-width: 40px; min-height: 40px; display: inline-flex; align-items: center; justify-content: center; padding: 8px; } .back-prev { bottom: 230px; } .player-row { grid-template-columns: auto auto auto 1fr; } .player-options { grid-column: 1 / -1; } .dock-title { display: none; } .subtitle-line { white-space: normal; line-height: 1.3; } }
  </style>
</head>
<body id="top">
  <header class="page-header">
    <div class="inner">
      <p class="eyebrow">New Concept English 2</p>
...
${renderPlayerDock(lesson.title, `../${lesson.audioFile}`, lesson.subtitles, previousHref, nextHref)}
  <script>
...
<body>
  <main class="shell">
    <div class="topbar">
      <div>
        <h1>NCE2 HTML 分析</h1>
        <p>第二册已生成 ${lessons.length} 课，美音发音、连读分析、句首播放和底部字幕播放器。生成过程逐课更新。</p>
      </div>
...
if (!lessons.length) throw new Error(`No NCE2 lessons found for ${startLesson}-${endLesson}`);
...
try {
  ...
  const previousHref = lessons[lessonIndex - 1]?.file.replace(/\.lrc$/, '.analysis.html') ?? null;
  const nextHref = lessons[lessonIndex + 1]?.file.replace(/\.lrc$/, '.analysis.html') ?? null;
  fs.writeFileSync(path.join(outDir, htmlName), renderHtml(lesson, analyses, previousHref, nextHref));
  written += 1;
  renderIndex();
  saveCaches();
  console.log(`generated NCE2 ${String(lesson.lessonNo).padStart(2, '0')}/${String(endLesson).padStart(2, '0')}: ${lesson.title} (${analyses.length} sentences)`);
...
console.log(`generated ${written} NCE2 lessons in ${outDir}`);
